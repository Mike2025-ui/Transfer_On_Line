...
void main() {
  ...
  List<MethodCall> mockNativeCalls({bool accessibilityEnabled = true}) {
  ...
  test(
    'happy path: NEW_FIELD -> INPUT, FINAL_FIELD -> DONE, RESULT -> session closed',
    () async {
      ...
      await runner.start(
        transactionReference: 'TOL-INT-1',
        attemptId: 7,
        ussdCode: '*123#',
        simSlot: 0,
      );
      ...
      expect(
        ...
        contains('startInteractiveUssdSession'),
      );

      bridge.emitUssdStepEvent(
        UssdStepEvent.fromChannelMap({'type': 'NEW_FIELD', 'fieldCount': 1}),
      );
      ...
      bridge.emitUssdStepEvent(
        UssdStepEvent.fromChannelMap({'type': 'FINAL_FIELD'}),
      );
      ...
      bridge.emitUssdStepEvent(
        UssdStepEvent.fromChannelMap({
          'type': 'RESULT',
          'status': 'SUCCESS',
          'operatorMessage': 'Transfert reussi',
        }),
      );
      ...
      expect(
        await queue.currentInteractiveSession(),
        isNull,
        reason: 'the Gateway must be free for the next interactive task',
      );
  ...
  test(
    'idempotency: the same key is reused verbatim across a retried event, never regenerated',
    () async {
      final queue = freshQueue();
      mockNativeCalls();
      ...
      final api = GatewayApi(
        client: buildStepClient((body, key) {
          if (body['event'] == 'NEW_FIELD') {
            ...
            attempts++;
            if (attempts < 2) {
              return null; // first attempt fails (network/server error)
            }
            return {'action': 'DONE'};
          }
          return {'action': 'ACK'};
      ...
      await runner.start(
        transactionReference: 'TOL-INT-2',
        attemptId: 8,
        ussdCode: '*123#',
      );
      bridge.emitUssdStepEvent(
      ...
      expect(
        seenKeys.toSet(),
        hasLength(1),
        reason: 'a retried event must reuse the exact same Idempotency-Key',
      );
  ...
  test(
    'accessibility disabled: reports FAILED immediately, never arms a native session',
    () async {
      final queue = freshQueue();
      ...
      await runner.start(
        transactionReference: 'TOL-INT-3',
        attemptId: 9,
        ussdCode: '*123#',
      );
      ...
      expect(
        calls.map((c) => c.method),
        isNot(contains('startInteractiveUssdSession')),
      );
  ...
  test(
    'backend FAILED mid-session cancels the native session and reports RESULT',
    () async {
      final queue = freshQueue();
      final calls = mockNativeCalls();
      final api = GatewayApi(
      ...
      await runner.start(
        transactionReference: 'TOL-INT-4',
        attemptId: 10,
        ussdCode: '*123#',
      );
      bridge.emitUssdStepEvent(
      ...
      await pumpUntil(
        () => calls.any((c) => c.method == 'cancelInteractiveUssdSession'),
      );
  ...
  test(
    'native TIMEOUT event reports RESULT FAILED and always cleans up',
    () async {
      final queue = freshQueue();
      mockNativeCalls();
      final reportedBodies = <Map<String, dynamic>>[];
      ...
      await runner.start(
        transactionReference: 'TOL-INT-5',
        attemptId: 11,
        ussdCode: '*123#',
      );
      bridge.emitUssdStepEvent(
        UssdStepEvent.fromChannelMap({'type': 'TIMEOUT'}),
      );
      await pumpUntil(() => reportedBodies.isNotEmpty);
  ...
  test(
    'BUSY reflection: pendingTaskCount() is non-zero while a session is active, zero once it ends',
    () async {
      final queue = freshQueue();
      mockNativeCalls();
      final api = GatewayApi(
      ...
      expect(await queue.pendingTaskCount(), 0);
      await runner.start(
        transactionReference: 'TOL-INT-6',
        attemptId: 12,
        ussdCode: '*123#',
      );
      expect(
        await queue.pendingTaskCount(),
        greaterThan(0),
        reason: 'the heartbeat must see the Gateway as busy',
      );
  ...
  test(
    'a second start() call while a session is already running is a no-op',
    () async {
      final queue = freshQueue();
      final calls = mockNativeCalls();
      final api = GatewayApi(
      ...
      await runner.start(
        transactionReference: 'TOL-INT-7',
        attemptId: 13,
        ussdCode: '*123#',
      );
      final callsAfterFirstStart = calls.length;
      await runner.start(
        transactionReference: 'TOL-INT-8',
        attemptId: 14,
        ussdCode: '*456#',
      );
      ...
      expect(
        ...
        reason: 'no new native call for the ignored second start()',
      );
      expect(
        (await queue.currentInteractiveSession())!.transactionReference,
        'TOL-INT-7',
      );
  ...
  test(
    'exhausted retries on a non-final event leave the session in place for the native timeout to resolve',
    () async {
      final queue = freshQueue();
      mockNativeCalls();
      ...
      await runner.start(
        transactionReference: 'TOL-INT-9',
        attemptId: 15,
        ussdCode: '*123#',
      );
      bridge.emitUssdStepEvent(
      ...
      expect(
        newFieldAttempts,
        2,
        reason: 'maxRetriesPerEvent honored, then gives up on this event',
      );
      expect(
        runner.isRunning,
        isTrue,
        reason:
            'must stay armed - only the native session timeout (or a later event) can end it',
      );
      expect(
        await queue.currentInteractiveSession(),
        isNotNull,
        reason: 'the pending event/key are preserved for a future retry path',
      );
  ...
  test(
    'a permanent rejection (409: attempt no longer active) is never retried and frees the Gateway immediately',
    () async {
      final queue = freshQueue();
      final calls = mockNativeCalls();
      var newFieldAttempts = 0;
        ...
        client: MockClient((request) async {
          ...
          if (body['event'] == 'NEW_FIELD') {
            ...
            return http.Response(
              jsonEncode({'error': 'Attempt is not active (status=failed)'}),
              409,
            );
          }
      ...
      await runner.start(
        transactionReference: 'TOL-INT-10',
        attemptId: 16,
        ussdCode: '*123#',
      );
      bridge.emitUssdStepEvent(
      ...
      expect(
        ...
        1,
        reason:
            '409 must never be retried - it is a permanent rejection, not a transient failure',
      );
      expect(
        calls.map((c) => c.method),
        contains('cancelInteractiveUssdSession'),
      );
      expect(
        await queue.currentInteractiveSession(),
        isNull,
        reason:
            'the Gateway must be freed immediately, not held for a futile retry budget',
      );
    },
  );

  group(
    'orphan recovery after a crash/reboot (gatewayServiceMain startup hook)',
    () {
      test(
        'an interactive_session row left over from a previous process is cleared, not reported',
        () async {
          final queue = freshQueue();
          final started = await queue.startInteractiveSession(
            transactionReference: 'TOL-ORPHAN',
            attemptId: 99,
          );
          ...
          expect(
            ...
            0,
            reason:
                'the Gateway must be free to accept a new interactive task after restart',
          );
        },
      );
