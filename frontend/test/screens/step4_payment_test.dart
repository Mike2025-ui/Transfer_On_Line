...
void main() {
  ...
  Future<Map<String, dynamic>> pumpAndConfirm(
    ...
    final client = MockClient((request) async {
      ...
      return http.Response(
        jsonEncode({
          'reference': 'TOL-TEST', 'status': 'pending', 'checkout_url': '',
          'payment_reference': 'PAY-TEST', 'payment_status': 'pending',
        }),
        201,
      );
    });
  ...
  testWidgets('tapping confirm sends the real operatorId/serviceId received from the screens above', (tester) async {
  ...
  testWidgets('different operatorId/serviceId values are forwarded faithfully, not hardcoded', (tester) async {
  ...
  testWidgets('the confirm button label and payment panels are unaffected by this change', (tester) async {
    final client = MockClient((request) async => http.Response(
          jsonEncode({'reference': 'TOL-TEST', 'status': 'pending', 'checkout_url': '', 'payment_reference': 'PAY-TEST', 'payment_status': 'pending'}),
          201,
        ));
    ...
    expect(find.text('Paiement CinetPay'), findsOneWidget);
    expect(find.text('Paiement GeniusPay'), findsOneWidget);
  ...
  testWidgets('double clic rapide sur PAYER : une seule requête POST /transactions/execute/ part', (tester) async {
    await tester.binding.setSurfaceSize(const Size(800, 1400));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    var postCalls = 0;
    final client = MockClient((request) async {
    ...
    await tester.pumpWidget(MaterialApp(
      home: Step4PaymentScreen(
        operatorId: 1, serviceId: 3, operator: 'Orange', service: 'Internet',
        operation: 'Souscription pour moi', phone: '0700000001', amount: 1000,
        onTransactionAdded: (Transaction _) {}, onNotificationAdded: (AppNotification _) {},
        notifications: const [],
        backendApiService: BackendApiService(client: client),
        authService: auth,
      ),
    ...
    expect(find.text('PAYER ET SOUSCRIRE'), findsNothing,
        reason: 'the button must not be re-actionable while the request is in flight');
    expect(find.byType(CircularProgressIndicator), findsOneWidget);

    await tester.pump(const Duration(milliseconds: 100));

    expect(postCalls, 1, reason: 'a rushed double tap must never create two transactions');
  });
  ...
  testWidgets('la même Idempotency-Key est réutilisée si l\'utilisateur retente depuis le même écran', (tester) async {
    await tester.binding.setSurfaceSize(const Size(800, 1400));
    addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    await tester.tap(find.text('PAYER ET SOUSCRIRE'));
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 50)); // first attempt fails (500), button re-enabled
    ...
    expect(seenKeys[0], seenKeys[1], reason: 'a retry from the same screen instance must reuse the exact same key');
  });
}
