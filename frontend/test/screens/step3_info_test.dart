...
void main() {
  ...
  testWidgets('shows the existing default amounts immediately - no loading state, ever', (tester) async {
  ...
  testWidgets('replaces the quick-amount tiles when the backend has specific amounts configured', (tester) async {
    ...
    expect(find.text('Autre'), findsOneWidget, reason: 'custom amount entry stays available even with a fixed catalog');
  ...
  testWidgets('an empty amounts list leaves the default tiles untouched', (tester) async {
    final client = MockClient((request) async => http.Response(jsonEncode([]), 200));
  ...
  testWidgets('a network error leaves the default tiles untouched - no error message shown', (tester) async {
  ...
  testWidgets('tapping "Autre" still opens the custom amount dialog', (tester) async {
    final client = MockClient((request) async => http.Response(jsonEncode([]), 200));
  ...
  testWidgets('continuing navigates to Step4PaymentScreen with the same arguments as before', (tester) async {
    final client = MockClient((request) async => http.Response(jsonEncode([]), 200));
