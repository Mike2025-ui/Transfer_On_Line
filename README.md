...
Application de souscription et transfert de forfaits avec Flutter, Django REST Framework, PostgreSQL, Redis, Jèko, GeniusPay et une Gateway Android chargée d'exécuter les codes USSD après confirmation de paiement.
...
Jèko est le fournisseur de paiement principal, avec GeniusPay comme relais. Le client crée une transaction via `POST /api/transactions/execute/`, l'API initialise le checkout et retourne `checkout_url`. La Gateway Android ne reçoit une transaction dans `GET /api/transactions/pending/` qu'après confirmation webhook du paiement.
...
2. Renseigner les clés Jèko/GeniusPay et les domaines de production.
